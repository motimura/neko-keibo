...
`utils/productCatalog.ts` に in-memory の静的配列でカタログを保持。SQLite は使わない。

現在は主要41ブランド（プレミアムフード26 + メインストリームフード5 + 消耗品10社）のテンプレート中心カタログ 96件。ロイヤルカナン、ニュートロ、ピュリナワンなどは詳細商品を網羅せず、容量別の「詳細を追記」候補を優先する。
